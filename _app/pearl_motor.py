...
class PearlMotor:
    ...
    def _build_cmd(self, wallet: str, pool_url: str, gid: str | None, boost: int = 3) -> list[str]:
        cmd = [
            str(self.exe),
            "--algorithm",
            "pearlhash",
    ...
    def _watch_failover(
        ...
        cmd = self._build_cmd(wallet, ssl_url, gid, int((__import__('json').loads((self.aura_root/'config.json').read_text(encoding='utf-8')).get('pearl_ai_boost') or 3)) if (self.aura_root/'config.json').is_file() else 3)
    ...
    def start(self, wallet: str, host: str, port: int, worker: str = "Aura", payout_coin: str | None = None, payout_address: str | None = None) -> None:
        ...
        try:
            import json
            _cfgp = self.aura_root / "config.json"
            _cfg = json.loads(_cfgp.read_text(encoding="utf-8")) if _cfgp.is_file() else {}
            boost = int(_cfg.get("pearl_ai_boost") or _cfg.get("ai_intensity") or 3)
        except Exception:
            boost = 3
        boost = max(1, min(8, boost))
        ...
        cmd = self._build_cmd(w, pool_arg, gid, boost)
        self._stop.clear()
        ...
        if self._proc.poll() is not None:
            ...
            cmd2 = self._build_cmd(w, pool_arg, None, boost)
            self._proc = self._popen(cmd2)
            self._reader = threading.Thread(target=self._read_loop, daemon=True)
            self._reader.start()
            time.sleep(1.0)
            if self._proc.poll() is not None:
                ...
                cmd3 = self._build_cmd(w, primary, gid, boost)
