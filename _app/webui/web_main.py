...
class Api:
    def __init__(self) -> None:
        ...
        self.pearl_ai = PearlAI(
            config_path=CONFIG_PATH,
            aura_root=CONFIG_PATH.parent,
            on_log=self._on_log,
            get_stats=lambda: dict(self._last_stats),
            restart_miner=self._ai_restart_pearl,
            on_notify=self._ai_notify,
        )
    ...
    def _ai_notify(self, title: str, body: str) -> None:
        try:
            import subprocess
            title = (title or "Aura AI").replace("'", "").replace("\n", " ")[:80]
            ...
            ps = (
                ...
                "[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Aura Miner').Show($toast);"
            )
            subprocess.Popen(
    ...
    def _ai_restart_pearl(self) -> None:
        """AI hunter applies new boost: quick restart Pearl miner only."""
        try:
            if not getattr(self, "pearl", None):
                return
            cfg = load_config()
            wallets = cfg.get("wallets") if isinstance(cfg.get("wallets"), dict) else {}
            address = str(wallets.get("PRL") or cfg.get("address") or "").strip()
            btc = str(wallets.get("BTC") or "").strip()
            host = str(cfg.get("host") or "pearlpow.unmineable.com")
            port = int(cfg.get("port") or 4444)
            worker = str(cfg.get("worker_name") or "Aura")
            pay = str(cfg.get("payout") or "BTC").upper()
            pay_coin, pay_addr = "BTC", btc
            if address.lower().startswith("prl1"):
                pay_coin, pay_addr = "PRL", address
            elif btc:
                pay_coin, pay_addr = "BTC", btc
            was = self.pearl.is_running()
            if was:
                try:
                    self.pearl_ai.stop()
                except Exception:
            ...
            if was or True:
                self.pearl.start(
                    ...
                    port,
                    worker=worker,
                    payout_coin=pay_coin,
                    payout_address=pay_addr,
                )
        except Exception as exc:
            self._on_log(f"AI restart pearl: {exc}")
    ...
    def get_state(self) -> dict:
        ...
        try:
            srb = summarize_srb()
            self._srb_live = srb if srb.get("ok") else {}
            if srb.get("ok"):
                if float(srb.get("hashrate_hs") or 0) > 0:
                    stats["hashrate_hs"] = float(srb["hashrate_hs"])
                    stats["hashrate"] = srb.get("hashrate") or stats.get("hashrate")
                if int(srb.get("accepted") or 0) >= int(stats.get("accepted") or 0):
                    stats["accepted"] = int(srb.get("accepted") or 0)
                    stats["rejected"] = int(srb.get("rejected") or 0)
                    stats["total"] = int(srb.get("total") or 0)
                stats["engine"] = srb.get("engine") or stats.get("engine")
                if int(srb.get("uptime") or 0) > 0:
                    mining_srb = True
        except Exception:
            self._srb_live = {}
        rate_hs = float(stats.get("hashrate_hs") or 0)
        try:
            mining = bool(mining or mining_srb)
        except Exception:
            pass
        peak = float(cfg.get("hs_peak") or 0)
        ...
        srb_up = int((getattr(self, "_srb_live", {}) or {}).get("uptime") or 0)
        if srb_up > uptime:
            uptime = srb_up
        ...
        return {
            ...
            "gpu_temp": (getattr(self, "_srb_live", {}) or {}).get("temp"),
            "gpu_power": (getattr(self, "_srb_live", {}) or {}).get("power"),
            "gpu_fan": (getattr(self, "_srb_live", {}) or {}).get("fan"),
            "pool_latency": (getattr(self, "_srb_live", {}) or {}).get("latency_ms"),
            "pool_difficulty": (getattr(self, "_srb_live", {}) or {}).get("difficulty"),
            "last_job_sec": (getattr(self, "_srb_live", {}) or {}).get("last_job_sec"),
            "ai_boost": (self.pearl_ai.current_boost() if getattr(self, "pearl_ai", None) else 0),
            "ai_on": bool(getattr(self, "pearl_ai", None) and self.pearl_ai.is_running()),
            "miner_uptime": int((getattr(self, "_srb_live", {}) or {}).get("uptime") or 0),
    ...
    def apply_mode(self, mode: str = "pool", coin_code: str = "") -> dict:
        """Switch pool/solo and auto-fill host/port/password for active coin."""
        cfg = load_config()
        mode = "solo" if str(mode).lower() == "solo" else "pool"
        coin = (coin_code or cfg.get("coin") or "PRL").strip().upper()
        try:
            c = get_coin(coin)
        except Exception as exc:
            return {"ok": False, "error": str(exc)}
        pair = c.get("solo" if mode == "solo" else "pool") or ("", "")
        host = str(pair[0] or "")
        port = str(pair[1] or "")
        cfg["coin"] = coin
        cfg["mode"] = mode
        cfg["host"] = host
        cfg["port"] = port
        if not str(cfg.get("pool_password") or "").strip():
            cfg["pool_password"] = str(c.get("password") or "x")
        # Pearl pool hint
        if coin == "PRL" and mode == "pool":
            cfg["host"] = host or "pearlpow.unmineable.com"
            cfg["port"] = port or "4444"
            cfg["payout"] = cfg.get("payout") or "BTC"
        if coin == "PRL" and mode == "solo":
            cfg["host"] = host or "solo-prl.2miners.com"
            cfg["port"] = port or "1919"
        save_config(cfg)
        self.cfg = cfg
        self._on_log(f"Mode {mode.upper()} · {cfg['host']}:{cfg['port']} · {coin} (auto popunjeno)")
        return {
            "ok": True,
            "mode": mode,
            "host": cfg["host"],
            "port": cfg["port"],
            "coin": coin,
            "pool_password": cfg.get("pool_password") or "x",
            "settings": {
                "coin": coin,
                "mode": mode,
                "host": cfg["host"],
                "port": str(cfg["port"]),
                "wallet": str((cfg.get("wallets") or {}).get(coin) or cfg.get("address") or ""),
                "worker_name": str(cfg.get("worker_name") or "Aura"),
                "pool_password": str(cfg.get("pool_password") or "x"),
                "payout": str(cfg.get("payout") or "BTC"),
            },
        }
        ...
        with self._lock:
            ...
            try:
                ...
                if code == "PRL" or algo == "pearlpow":
                    ...
                    try:
                        self.pearl_ai.start()
                        self._on_log("Aura AI hunter aktivan — lovi jobove/blok-kandidate na PearlPow")
                    except Exception as ai_exc:
                        self._on_log(f"Pearl AI start: {ai_exc}")
    ...
    def save_settings(self, payload: dict) -> dict:
        try:
            ...
            cfg["coin"] = coin
            cfg["mode"] = mode
            # AUTO_FILL_ENDPOINT: pool<->solo always packs host/port/login defaults
            try:
                c = get_coin(coin)
                pair = c.get("solo" if mode == "solo" else "pool")
                # Always overwrite host/port on mode change so user gets ready-to-start values
                if pair and pair[0]:
                    host = str(pair[0])
                    port = str(pair[1] if len(pair) > 1 else port)
                if not str(cfg.get("pool_password") or "").strip():
                    cfg["pool_password"] = str(c.get("password") or "x")
            except Exception:
                pass
    ...
    def get_withdraw_state(self) -> dict:
        """POVUCI modal: show LIVE pool balance for payout coin (mining wallet)."""
        cfg = load_config()
        self.cfg = cfg
        ...
        if pay == "NATIVE" or pay not in PAYOUT_COINS:
            pay = "BTC"
        wallets = cfg.get("wallets") if isinstance(cfg.get("wallets"), dict) else {}
        # Balance lives on the mining/payout address on unMineable, NOT withdraw dest
        mine_addr = str(wallets.get(pay) or wallets.get("BTC") or cfg.get("address") or "").strip()
        dest = self._withdraw_address(pay)
        ...
        thr = None
        bal_text = "—"
        hint = "Saldo se čita s unMineable mining walleta. Isplata kad dosegne minimum."
        try:
            if mine_addr:
                data = fetch_unmineable_payout(pay, mine_addr)
                if isinstance(data, dict) and not data.get("error"):
                    bal = data.get("balance")
                    rawd = data.get("raw") if isinstance(data.get("raw"), dict) else {}
                    nested = rawd.get("data") if isinstance(rawd.get("data"), dict) else rawd
                    if isinstance(nested, dict):
                        thr = nested.get("payment_threshold")
                        if bal is None:
                            bal = nested.get("balance") or nested.get("balance_payable")
                    try:
                    ...
                    try:
                        ...
                        thr_f = None
                    if bal_f is not None and thr_f and thr_f > 0:
                        pct = 100.0 * bal_f / thr_f
                        bal_text = f"{bal_f:.8f} {pay}  ·  do isplate {pct:.2f}% (min {thr_f:g} {pay})"
                    elif bal_f is not None:
                        bal_text = f"{bal_f:.8f} {pay}"
                    else:
                        bal_text = f"{bal} {pay}" if bal is not None else "—"
                    hint = (
                        f"Mining wallet na poolu: {mine_addr[:10]}…{mine_addr[-6:]}. "
                        "POVUCI sprema adresu za isplatu; unMineable šalje kad prođe minimum."
                    )
                elif isinstance(data, dict) and data.get("error"):
                    hint = str(data.get("error"))
                    bal_text = "greška / nema salda"
            else:
                hint = f"Nema {pay} mining adrese u Settings — upiši wallet pa Refresh."
                bal_text = "nema mining walleta"
        except Exception as e:
            hint = f"Saldo greška: {e}"
            bal_text = "—"
        # Prefer showing mining addr in modal if dest empty
        show_addr = dest or mine_addr
        return {
            "ok": True,
            "payout": pay,
            "address": show_addr,
            "mining_address": mine_addr,
            "tag": tag,
            "balance": bal,
            "threshold": thr,
            "balance_text": bal_text,
            "hint": hint,
            "coins": list(PAYOUT_COINS) if "PAYOUT_COINS" in dir() else ["BTC", "BCH", "PRL", "XRP", "XMR"],
        }
...
def main() -> None:
    ...
    kw = dict(js_api=api, width=1100, height=780, min_size=(960, 700), background_color="#0a0612")
    webview.create_window("Aura Miner", index, **kw)
