...
function setMode(m, applyServer) {
  mode = m === "solo" ? "solo" : "pool";
  document.querySelectorAll("#modeSeg button, #dashModeSeg button").forEach((b) => {
    b.classList.toggle("on", b.dataset.mode === mode);
  });
  fillEndpointFromCoin();
  updateLoginPreview();
  if (applyServer !== false) {
    api("apply_mode", mode, ($("setCoin") && $("setCoin").value) || ($("coinSelect") && $("coinSelect").value) || "")
      .then((res) => {
        if (res && res.ok && res.settings) {
          if ($("setHost")) $("setHost").value = res.settings.host || "";
          if ($("setPort")) $("setPort").value = res.settings.port || "";
          if ($("setPass") && res.settings.pool_password) $("setPass").value = res.settings.pool_password;
          updateLoginPreview();
        }
        return refresh();
      })
...
function applyState(st) {
  ...
  if ($("gpuTemp")) $("gpuTemp").textContent = st.gpu_temp != null ? (st.gpu_temp + "°C") : "—";
  if ($("gpuPower")) $("gpuPower").textContent = st.gpu_power != null ? (st.gpu_power + " W · fan " + (st.gpu_fan ?? "—") + "%") : "—";
  if ($("poolLat")) $("poolLat").textContent = st.pool_latency != null ? (st.pool_latency + " ms") : "—";
  if ($("poolDiff")) $("poolDiff").textContent = st.pool_difficulty != null ? ("Diff " + st.pool_difficulty) : (st.last_job_sec != null ? ("Last job " + st.last_job_sec + "s") : "Diff —");
  if ($("aiState")) $("aiState").textContent = st.ai_on ? "ON" : "OFF";
  if ($("aiBoost")) $("aiBoost").textContent = "boost " + (st.ai_boost ?? "—");
  if ($("liveLine")) $("liveLine").textContent = st.running
    ? (`Live: ${fmtHs(st.hashrate_s != null ? st.hashrate_s : st.hashrate)} · ${acc} accepted · job ${st.last_job_sec ?? "?"}s`)
    : "Live: stopped";
  document.title = st.running ? (`Aura Miner · ${fmtHs(st.hashrate_s != null ? st.hashrate_s : st.hashrate)}`) : "Aura Miner";
...
$("coinSelect").addEventListener("change", async () => {
  ...
  fillEndpointFromCoin();
  await api("apply_mode", mode, $("coinSelect").value);
  await refresh();
});
document.querySelectorAll("#modeSeg button, #dashModeSeg button").forEach((b) => {
  b.addEventListener("click", () => { settingsDirty = true; setMode(b.dataset.mode, true); });
});
document.querySelectorAll("#nmActiveSeg button").forEach((b) => {
...
async function refreshPovuci() {
  try {
    ...
    const balBig = document.getElementById("povuciBalBig");
    if (balBig) balBig.textContent = st.balance_text || "—";
    const hint = document.getElementById("povuciHint");
    if (hint) hint.textContent = st.hint || "";
    if (msg && st.hint) msg.textContent = st.hint;
